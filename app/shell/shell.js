 ...
 *   Passed to `initPageNavPanel()`. `false` skips the nav chrome.
...
export function initShell(options = {}) {
  ...
  renderPageShell({ ...shellOptions, ...alsoSeeOptions, pageNav });
  initIcons();
  ...
  if (pageNav !== false) initPageNavPanel("#page-nav", pageNav);
